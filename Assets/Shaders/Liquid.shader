Shader "Unlit/Liquid"
{
	Properties
	{
		...
		_FilAmount ("Fill Amount", Range(-10,10)) = 0
	...
	SubShader
	{
		...
		Pass
		{
			...
			float4 RoateAroundYInDegrees(float4 vertex, float degrees)
			{
			...
			v2f vert (appdata v)
			{
				...
				float3 worldPosX = RoateAroundYInDegrees(float4(worldPos,0), 360);
				float3 worldPosZ = float3(worldPosX.y, worldPosX.z, worldPosX.x);
				float3 worldPosAdjusted = worldPos + (worldPosX * _WobbleX) + (worldPosZ * _WobbleZ);

				o.fillEdge = worldPosAdjusted.y + _FilAmount;
